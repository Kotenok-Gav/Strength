...
//import BoxGeometry from "../components/forms/createRocket/createRocket";
...
const Forms = () => {
  ...
  // Форма ввода
  function DropdownForm({ title, isOpen, toggleForm, content }) {
    return (
      <div>
        <button onClick={toggleForm}>
          <p className="pb-3 pl-5 text-left text-2xl">{title}</p>
        </button>
        {isOpen && (
          <form>
            {content}
  ...
  const formInitialStates = [
    { title: '1 Общие данные', content: <p>
      <form>
    ...
    { title: '2 Геометрические хар-ки', content: <p>
      <form>
    ...
    { title: '3 Параметры (ОВП)', content: <p>
      <form>
    ...
    { title: '4 Граничные условия', content: <p>
      <form>
    ...
    { title: '5 Распределение масс', content: <p>
      <form>
    ...
    { title: '6 Свойства материалов', content: <p>
      <form>
  ...
  const [formStates, setFormStates] = useState(formInitialStates.map(state => ({ isOpen: false, ...state })));
  const toggleForm = (index) => {
    const updatedFormStates = formStates.map((formState, i) => {
      if (i === index) {
        return { ...formState, isOpen: !formState.isOpen };
      }
      return { ...formState, isOpen: false };
    });
  ...
  {/* Кнопки------------------------------------------------------------ */}
            <div className="flex items-center flex-col ">
              <button
                ...
                className="p-5 rounded-3xl bg-slate-700 text-xl mt-10 w-1/2 hover:bg-sky-700"
              >
                Создать
              </button>
          ...
          <div className="one-window">
            <div className="fixed">
              <div>
                <h1 className="text-3xl text-center pb-5">Ракета</h1>
                <div className="rocket-three-D">
                  <Canvas>
                    <mesh>
          ...
          <div className="one-window">
            <div className="fixed">
              <h1 className="text-3xl text-center pb-5">Графики и данные</h1>
              <div>
                {graph}
              </div>
            </div>
